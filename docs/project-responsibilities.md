...
┌─────────────────────────────────────────────────────────────┐
│ 使用者界面层（未来）                                          │
│   eidolon-studio  可视化统一数据操作：角色创建 / 修改 / 导出     │
├─────────────────────────────────────────────────────────────┤
│ 运行时层（未来）                                              │
...
### 3.7 eidolon-studio（使用者界面层 · 未来）

| 项 | 内容 |
|----|------|
| 定位 | **可视化统一数据操作项目**——建立在底层各层之上的图形化壳 |
...
**不负责（规划）**
- 不重新定义数据格式（那是扩展层 / 协议层的职责）
- 不重复实现容器逻辑（那是 PersonaSeed 的职责）
- 不做运行 / 推理（那是运行时 / 人格层的职责）

> 开发阶段刻意将各项目分开，正是为了让本层未来能"即插即用地"复用每一块底层能力。底层协议与模块保持独立、可单独复用，Studio 只是它们的图形化合集。详见 [`resource-management.md` §5](./resource-management.md)。
...
| eidolon-studio | 未来（可视化统一操作） | `Eidolon/eidolon-studio/` | — |
